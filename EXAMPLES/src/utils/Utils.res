module ReasonPromise = {
  @ocaml.doc("
   Promises of result('a, 'e) coming from `rescript-apollo-client` will never reject.
   It's safe to simply cast these to a Promise.t(result('a, 'e)).
   See https://github.com/aantron/promise#Bindings
   ")
  external fromJs: Js.Promise.t<result<'a, 'e>> => Promise.t<result<'a, 'e>> = "%identity"
}
...
module Promise = {
  @ocaml.doc("
   Promises in rescript-apollo-client never reject, so we're not using catch here.
